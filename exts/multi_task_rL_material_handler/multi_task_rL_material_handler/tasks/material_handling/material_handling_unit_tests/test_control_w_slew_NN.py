...
"""Script to test how to control the material handler with the nn model for the slew in numpy"""
...
def main():
    ...
    means = [0., 7.78003071e+01, 7.78003071e+01, 0., 4.03212056e+05]  # new model
    stds = [4.10256637e+02, 9.77388791e+01, 9.77388791e+01, 3.45157123e-01, 1.50740475e+05]
    ...
    press_l = 0.
    press_r = 0.
    speed = np.zeros(5)  # stores slew, boom, stick, tooly, toolx
    position = np.zeros(5)
    position[1] = -0.8
    position[2] = 1.5
    old_speed = np.zeros(5)  # needed to simulate the pendulum

    input_buff = [-means[0]/stds[0]] * (history*stride + 1)
    pressl_buff = [-means[1]/stds[1]] * (history*stride + 1)
    pressr_buff = [-means[2]/stds[2]] * (history*stride + 1)
    speed_buff = [-means[3]/stds[3]] * (history*stride + 1)
    ...
    while simulation_app.is_running():
        ...
        for i in range(5):
            ...
            input_buff.insert(0, slew)
            input_buff.pop()

            pressl_buff.insert(0,0)
            pressl_buff.pop()
            pressr_buff.insert(0,0)
            pressr_buff.pop()
            speed_buff.insert(0, 0)   # need to already insert fake values to keep alignment
            speed_buff.pop()

            machine_inertia = compute_inertia_from_angles(-robot.data.joint_pos[:,1], -robot.data.joint_pos[:,2], 0)
            inertia = [( machine_inertia - means[4])/stds[4]]
            inertia = [float(inertia[0].numpy())]
            inertia = [(449225- means[4])/stds[4]]
            feature_press = np.concatenate((input_buff[0:history*stride:stride], \
                ...
                inertia), axis=0)
            feature_press = feature_press.astype(float)

            input_press = torch.from_numpy(feature_press)
            input_press = input_press.to('cuda')
            ...
            feature_speed = np.concatenate((input_buff[0:history*stride:stride], \
                pressl_buff[0:history*stride:stride], \
                pressr_buff[0:history*stride:stride], \
                speed_buff[stride:history*stride+1:stride], \
                inertia), axis=0)
            feature_speed = feature_speed.astype(float)

            input_speed = torch.from_numpy(feature_speed)
            input_speed = input_speed.to('cuda')
            
            vel_out = model_vel(input_speed.float())
            vel_out = vel_out.detach().cpu().numpy()
            speed_buff[0] = vel_out[0]

            press_l = (float)(press_out[0]*stds[1] + means[1])
            press_r = (float)(press_out[1]*stds[2] + means[2])
            old_speed[0] = speed[0]
            speed[0] = (float)(vel_out*stds[3] + means[3])
            position[0] = (float)(position[0] + speed[0]*0.02)
        ...
        slew_vel.append(speed[0])
        pressureL.append(press_l)
        pressureR.append(press_r)
...
def compute_inertia_from_angles(boomAngle, stickAngle, load):
    
    boomAngle = boomAngle.cpu()
    stickAngle = stickAngle.cpu()

    base_mass = 9710+6007
    base_dim = [3.2, 3.0, 1.5]

    cabin_mass = 1055+898
    cabin_dim = [1.8, 1.0, 1.7]

    arm1_mass = 3848+(154+103+176+117)*2
    arm1_dim = [10.0, 1.0, 1.0]

    arm2_mass = 1745
    arm2_dim = [7.0, 1.0, 0.5]
    ...
    I_com = base_mass*(base_dim[0]/2)**2 + cabin_mass*(cabin_dim[0]/2)**2 + arm1_mass*(arm1_dim[0]/2*np.cos(boomAngle))**2 + arm2_mass*(arm1_dim[0]*np.cos(boomAngle) - arm2_dim[0]/2*np.cos(boomAngle+stickAngle))**2 + gripper_mass*(arm1_dim[0]*np.cos(boomAngle) - arm2_dim[0]*np.cos(boomAngle+stickAngle))**2
    I_cub = cuboid_Icom(base_mass, base_dim, 0) + cuboid_Icom(cabin_mass, cabin_dim, 0) + cuboid_Icom(arm1_mass, arm1_dim, boomAngle) + cuboid_Icom(arm2_mass, arm2_dim, boomAngle+stickAngle); 
...
def cuboid_Icom(mass, dim, angle):
    temp = (dim[0]**2)*(np.cos(angle)**2)+(dim[2]**2)*(np.sin(angle)**2)+(dim[1]**2)
    return mass*temp/12
